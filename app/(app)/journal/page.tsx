...
function StatCard({ label, value, sub, icon, highlight = "neutral" }: StatCardProps) {
  ...
  return (
    <div className="flex items-center gap-3 p-4 rounded-xl border border-slate-800 bg-slate-900/50">
      <div className="p-2 rounded-lg bg-slate-800 text-slate-400">{icon}</div>
      <div>
        <p className="text-xs text-slate-500">{label}</p>
        <p className={`text-xl font-bold tabular-nums leading-tight ${colors[highlight]}`}>{value}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          highlight={winRate >= 50 ? "green" : winRate > 0 ? "neutral" : "neutral"}
        />
        <StatCard
          label="Total P&L"
          value={
